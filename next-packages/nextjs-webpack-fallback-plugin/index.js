...
class FallbackPlugin {
    ...
    getBelongingExtension(pathname) {
        ...
        /* if (pathname.includes('@virtual-module')) {
            const [module] = pathname.match(/@virtual-module[\/|\\]\w+/);
            return module;
        }
 */
        const { packageName } = extensions.entries.reduce(
            (acc, [packageName, sourcePath]) => {
                const { currentIndex } = acc;
                const sourcePathSrc = path.join(sourcePath, 'src');
                const sourcePathPublic = path.join(sourcePath, 'public');

                const srcIndex = pathname.indexOf(sourcePathSrc);
                const publicIndex = pathname.indexOf(sourcePathPublic);
                const index = srcIndex === -1 ? publicIndex : srcIndex;

                if (index > currentIndex) {
                    return { packageName, currentIndex: index };
                }

                return acc;
            },
            { packageName: '', currentIndex: -1 }
        );
